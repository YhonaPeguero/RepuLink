    ...
    >
      ...
      <div className="relative z-10 flex items-start justify-between gap-3">
        <div className="space-y-1">
          <h3
            className={`text-lg font-bold leading-tight ${isApproved ? "text-foreground" : "text-muted"}`}
          >
            {account.title}
          </h3>
          <p className="text-xs text-muted/80 font-mono tracking-wide">
            {formattedDate}
          </p>
        </div>
      ...
      <div className="relative z-10 mt-auto pt-4 border-t border-border-low/50">
        <div className="flex items-center gap-2 mb-3">
          <ShieldCheck
            className={`h-4 w-4 ${isApproved ? "text-primary" : "text-muted"}`}
          />
              ...
              <div className="flex flex-wrap gap-2">
                {account.clientLinkedin && (
                  <a
                    href={
                      "https://" +
                      account.clientLinkedin.replace(/^https?:\/\//, "")
                    }
                    target="_blank"
                    rel="noreferrer"
