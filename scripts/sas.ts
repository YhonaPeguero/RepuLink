 ...
 *
 * El nonce de cada atestación es el Job PDA, así la attestation PDA es
 * determinística por (credential, schema, job) y se puede buscar sin indexar.
 */
import {
...
// v1 permanece inmutable en devnet. v2 hace que la attestation conserve las
// partes y la economía básica aunque el cliente cierre después la cuenta Job.
export const SCHEMA_VERSION = 2;
// Layout compacto SAS: 12=string, 0=u8, 8=i64, 3=u64 (ver sas-lib utils).
export const SCHEMA_LAYOUT = new Uint8Array([12, 0, 8, 8, 12, 12, 12, 3]);
...
/** Payload autosuficiente del schema repulink-job v2. */
...
export async function loadAuthority({
  allowCreate = false,
}: { allowCreate?: boolean } = {}): Promise<KeyPairSigner> {
  ...
  if (!Array.isArray(parsed) || parsed.length !== 32) {
    throw new Error(
      `${KEY_PATH} no es una keypair válida (se esperan 32 bytes)`
    );
  }
...
export async function sendIxs(
  ...
  await rpc
    ...
    .send();
  // El WebSocket del RPC público de devnet es inestable; confirmar por polling
  // evita que la emisión de SAS se quede colgada justo antes de una demo.
  await awaitSignatureCommitment(rpc, signature, "confirmed");
  return signature;
}
...
export async function emitJobAttestation(
  ...
  if (existing.exists) {
    ...
    console.log(
      `atestación ya existe con el payload esperado: ${attestationPda}`
    );
